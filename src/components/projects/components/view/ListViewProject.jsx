...
const ListViewProject = ({ projectDetails }) => {
    ...
    const [measureHeight, setMeasureHeight] = useState(
        window.innerHeight >= comparableHeight
    );
    const [measureWidth, setMeasureWidth] = useState(
        window.innerWidth >= comparableHeight
    );
    const [imgSize, setImgSize] = useState("");

    useMotionValueEvent(scrollYProgress, "change", (latest) => {
        const cardsBreakpoints = projectDetails.map(
            (_, index) => index / cardLength
        );
        cardsBreakpoints.forEach((breakpoint, index) => {
            if (latest > breakpoint - 0.06 && latest <= breakpoint) {
    ...
    return (
        <motion.div
            className={`${
                measureHeight ? "h-[37rem]" : "h-[28rem]"
            } overflow-y-auto flex flex-col lg:flex-row justify-center space-x-10 rounded-md px-10 rin  sticky top-20`}
            ref={ref}
        >
            <motion.div
            ...
            >
                <div className="w-full h-full py-10">
                    {projectDetails.map((item, index) => (
                        <div
                            className={`mt-16 mb-[9rem] 2xl:mt-[2rem] w-full ring-[1px] p-4 rounded-md ${
                                mode === "dark"
                                    ? "bg-[#0f203baf]"
                                    : "bg-[#a8c8fcc8]"
                            } `}
                        ...
                        >
                            <motion.h2
                                ...
                                className={`text-2xl font-bold ${
                                    mode === "dark"
                                        ? "text-slate-100 border-cyan-300"
                                        : "text-slate-800 border-cyan-700"
                                } border-b pb-1 tracking-wide`}
                            >
                                {item.name}
                            </motion.h2>
                            ...
                            <motion.div
                                className="mt-9 flex flex-row-reverse items-center justify-between"
                                initial={{ opacity: 0 }}
                                animate={{
                                    opacity: activeCard === index ? 1 : 0.3,
                                }}
            ...
            <div
                className={`${
                    measureHeight ? "max-h-[40rem]" : "h-full"
                } xl: hidden lg:flex w-3/5 items-center justify-center sticky top-0 overflow-hidden`}
            >
                <motion.div
                    key={activeCard}
                    initial={{ opacity: 0.7, scale: 1, x: 500 }}
                    animate={{
                        opacity: 1,
                        scale: 1,
                        x: 0,
