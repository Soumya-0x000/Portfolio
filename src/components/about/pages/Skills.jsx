...
const generateSquares = (mode, isShuffle) => {
    return shuffle(skillsData, isShuffle).map((sq) => (
        <motion.div
            layout
        ...
        >
            <div
                className={`bg-slate p-2 rounded-full h-full w-full flex items-center justify-center overflow-hidden ${
                    mode === "dark"
                        ? "hover:bg-slate-800"
                        : "hover:bg-slate-200"
                } transition-all`}
    ...
    return (
        <div className="flex items-center flex-col gap-y-8 sm:gap-y-[50px] mt-20 pb-24">
            <span className="flex items-center justify-center bo rounded-full">
                <motion.span
                    className={`px-6 md:px-7 xl:px-9 py-1 md:py-2 xl:py-3 text-lg sm:text-xl lg:text-[25px] font-semibold rounded-full ${
