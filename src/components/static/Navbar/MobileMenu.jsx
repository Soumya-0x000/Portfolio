...
const Option = ({ text, icon, link, mode, setOpen }) => {
    ...
    return (
        ...
        onClick={handleCLick}
        className={`flex items-center gap-2 w-full p-2 sm:p-3 text-sm sm:text-[17px] font-medium whitespace-nowrap rounded-md hover:font-bold ${mode === 'dark' ? 'text-blue-300 hover:text-indigo-600 hover:bg-indigo-200' : 'text-indigo-800 hover:bg-indigo-300'} transition-colors cursor-pointer`}>
    ...
    return (
        <div className="flex items-center justify-center">
            <motion.div animate={open ? "open" : "closed"} className="relative">
                ...
                className={`flex flex-col gap-2 p-2 rounded-lg  ${mode === 'dark' ? 'bg-blue-950' : 'bg-blue-200'} shadow-xl w-48 sm:w-[20rem] md:w-[26rem] fixed left-1/2 -translate-x-1/2 top-[40%] -translate-y-1/2`}>
