...
const navigationLink = [
    {
        ...
        icon: <IoHomeOutline />,
        title: "Home",
        onHoverLight: "hover:text-yellow-600",
        onHoverDark: "hover:text-yellow-200",
        undrlnBGLight: "bg-yellow-700",
        undrlnBGDark: "bg-yellow-200",
    },
    {
        link: "/about",
        icon: <FaRegAddressCard />,
        title: "About",
        onHoverLight: "hover:text-cyan-600",
        onHoverDark: "hover:text-cyan-300",
        undrlnBGLight: "bg-cyan-700",
        undrlnBGDark: "bg-cyan-300",
    },
    {
        link: "/projects",
        icon: <GoProjectSymlink />,
        title: "Projects",
        onHoverLight: "hover:text-rose-600",
        onHoverDark: "hover:text-rose-300",
        undrlnBGLight: "bg-rose-700",
        undrlnBGDark: "bg-rose-300",
    },
    {
        link: "/education",
        icon: <MdOutlineArticle />,
        title: "Education",
        onHoverLight: "hover:text-green-600",
        onHoverDark: "hover:text-green-300",
        undrlnBGLight: "bg-green-700",
        undrlnBGDark: "bg-green-300",
    },
    {
        link: "/feedback",
        icon: <VscFeedback />,
        title: "Feedback",
        onHoverLight: "hover:text-lime-600",
        onHoverDark: "hover:text-lime-300",
        undrlnBGLight: "bg-lime-700",
        undrlnBGDark: "bg-lime-300",
    },
];
const mobileMenuLinks = navigationLink.map(({ link, icon, title }) => ({
    link,
    icon,
    title,
}));
...
const Navbar = () => {
    ...
    const [btnText, setBtnTxt] = useState(
        window.innerWidth <= 1024 ? "PORTFOLIO" : "SSD PORTFOLIO"
    );
    const [navActive, setNavActive] = useState(
        Array(navigationLink.length).fill(false)
    );
    ...
    return (
        <div
            className={`flex items-center justify-between w-full px-1 2xl:px-28 py-2 ${
                mode === "dark"
                    ? "bg-darkBlue text-light border-b border-b-blue-400"
                    : "border-b-lighter bg-violet-100"
            } z-40 fixed`}
        >
            <motion.div
                                ...
                                >
                                    <span
                                        className={`flex items-center justify-center gap-x-1 ${
                                            mode === "dark"
                                                ? `text-violet-300 ${item.onHoverDark}`
                                                : `text-violet-800 ${item.onHoverLight}`
                                        }`}
                                    >
                                    ...
                                    <span
                                        className={`${underlineColor(
                                            index
                                        )} absolute h-[1px] -bottom-0.5 left-0 ${
